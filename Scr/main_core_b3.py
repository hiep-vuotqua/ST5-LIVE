# main_core_b3.py — scan + Telegram alert
import json, os, time, requests
import pandas as pd, numpy as np
from datetime import datetime
from config_core_b3 import (
    CORE_B3, VOLUME_RATIO_MIN, ROC10_MIN, MACD_HIST_MIN, ADX14_MIN,
    USE_MA200, USE_MA50, MIN_HOLD_DAYS, FEE_PER_ROUND,
    TIMEZONE, STATE_FILE, TELEGRAM_TITLE, DELAY_BETWEEN_TICKERS
)
from data_core_b3 import get_data
...
def check_signal(df):
    """Check nến cuối cùng đã đóng (hôm qua)."""
    last = df.iloc[-1]
    buy = (
        last["volR"] >= VOLUME_RATIO_MIN and
        last["roc10"] >= ROC10_MIN and
        last["macd_hist"] > MACD_HIST_MIN and
        last["adx14"] >= ADX14_MIN and
        ((not USE_MA200) or last["close"] > last["ma200"]) and
        ((not USE_MA50)  or last["close"] > last["ma50"])
    )
    return {
        "buy": bool(buy),
...
def main():
    ...
    print(f"[{TELEGRAM_TITLE}] {today} — {len(CORE_B3)} mã")
    ...
    for tk in CORE_B3:
        try:
            ...
            if sig["buy"] and not t_state["in_position"]:
                if sig["date"] not in t_state["alerted_buy_dates"]:
                    alerts.append(
                        ...
                        f"   nến {sig['date']} | ADX={sig['adx']:.1f} VolR={sig['volR']:.2f} ROC={sig['roc10']:.1f}"
                    )
            ...
            if t_state["in_position"] and t_state["buy_date"]:
                ...
                if bars_after >= MIN_HOLD_DAYS + 1:
                    alerts.append(
                        f"🔴 BÁN <b>{tk}</b> @ {sig['close']:.2f} (giữ {bars_after} nến)"
                    )
